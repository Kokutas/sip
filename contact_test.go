...
func TestContact_Raw(t *testing.T) {
	ms := []*Contact{
		...
		NewContact("display name", "", "sips", "34020000001320000001", "192.168.0.1", 5060, "", 0, sync.Map{}),
		NewContact("display name", "", "sips", "34020000001320000001", "192.168.0.1", 5060, "", -1, sync.Map{}),
	}
	for _, m := range ms {
		result := m.Raw()
